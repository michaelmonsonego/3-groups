...
saveRDS(myeloid, file = "//asafmadilabfs/asafmadilab$/michael/Madi lab/scRNA seq Pipeline/m/sara_teva/ateno+saline/Objects/Teva_myeloid_0_before_dim_object_2.rds")
# dim+res choosing
myeloid = FindNeighbors(myeloid, dims = 1:12)
myeloid = FindClusters(myeloid, resolution = 0.25)
myeloid = RunTSNE(myeloid, dims = 1:12, verbose = F)
DimPlot(myeloid, reduction = "tsne", label= TRUE, pt.size=0.5)
ggsave(file = "Plots/Myeloid/myeloid/tSNE.png", dpi=300, width=12, height=12)

TSNEPlot(myeloid, group.by ="Treatment", cols= c("#CFBAE1", "#5C7D9D","#A4DEF9"), pt.size=0.8)
ggsave(file = "Plots/Myeloid/myeloid/tSNE - by treatment.png", dpi=300, width=12, height=12)

table(myeloid$orig.ident)["A"] #  1306 
table(myeloid$orig.ident)["D"] #  2980 
```

### make sure no T cells
```{r message=FALSE, warning=FALSE, cache= TRUE, fig.width=16, fig.height= 18}
FeaturePlot(myeloid, features = c("Ptprc", "Cd3e", "Cd8a", "Cd4", "Foxp3"), order=TRUE,pt.size=1, reduction="tsne", ncol=2)
ggsave(file = "Plots/Myeloid/myeloid/T_check.png", dpi=300, width=30, height=20)
```
...
write_csv(Top50Markers_myeloid, "//asafmadilabfs/asafmadilab$/michael/Madi lab/scRNA seq Pipeline/m/sara_teva/ateno+saline/Excels/memory_myeloid_DE_genes.csv")
...
ggsave(file = "Plots/Myeloid/myeloid/myeloid cells markers - heatmap.png", dpi=300, width=12, height=20)

saveRDS(myeloid, file="//asafmadilabfs/asafmadilab$/michael/Madi lab/scRNA seq Pipeline/m/sara_teva/ateno+saline/Objects/Teva_myeloid_intermediate_object.rds")
```