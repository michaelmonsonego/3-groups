...
ggsave(file = "Plots/T/Havcr2-3.png", dpi=300, width=8, height=6)
...
geom_boxplot(alpha=0.3,show.legend = FALSE, outlier.shape = NA)
ggsave(file = "Plots/T/Havcr2-ateno_saline.png", dpi=300, width=8, height=6)
...
VlnPlot(ateno_saline, features = c('Havcr2'), flip= TRUE, split.by = "Treatment",pt.size = 0)+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1), axis.title.x = element_blank())+
   scale_fill_manual(values= c("#CFBAE1","#A4DEF9"))+
   geom_boxplot(alpha=0.3,show.legend = FALSE, outlier.shape = NA)+
   annotate("text", x = x_levels, y=1.1, label = p_value_asterisks_bonferroni, size = 6)
ggsave(file = "Plots/T/Havcr2-ateno_saline_with_sig_test.png", dpi=300, width=8, height=6)
