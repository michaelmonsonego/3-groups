...
il2_sig <- make_signature(Tcells, il2,idents = c("A","B","D"), format_flag = TRUE)
...
```{r}
CD3subset <- readRDS(file="//asafmadilabfs/asafmadilab$/michael/Madi lab/scRNA seq Pipeline/m/sara_teva/3_groups/Objects/T cells/Teva_T_intermediate_object.rds"))

levels(CD3subset$Sample) <- c('Saline','αPD1(RMP1-30)-IL2')
...
signatures <- c('Tcells_Cytotoxicity', 'Tcell_chemotaxis','Tcell_Integrin_activation',
                ...
                'Tcells_Cytotoxicity','NF_KAPPAB_SIGNALING', 'Tregs_suppression')
...
for (name in signatures){
    ...
    print(name)
    CD3subset.sig <- make_signature(CD3subset.sig, name = name, up = get(name),title = name)[[1]]
    CD3subset.sig[[name]] = (CD3subset.sig[[name]]-min(CD3subset.sig[[name]]))/
      (max(CD3subset.sig[[name]])-min(CD3subset.sig[[name]]))
}

saveRDS(CD3subset.sig,"Objects//CD3subset.sig.RDS")
