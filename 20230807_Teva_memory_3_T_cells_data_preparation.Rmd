...
merged = readRDS("//asafmadilabfs.tau.ac.il/asafmadilab$/Sarah/experiment/teva/memory/20221225 - memory - scRNA seq/R/3 groups/Objects/Teva_merged_object.rds")
...
ggsave(file = "Elbow plot_0.png", dpi=300, width=6, height=6)

T_0_intermediate = saveRDS(T_cells_0, file="//asafmadilabfs.tau.ac.il/asafmadilab$/Sarah/experiment/teva/memory/20221225 - memory - scRNA seq/R/3 groups/Objects/Teva_T_0_intermediate_object.rds")
```
...
ggsave(file = "tSNE_0.png", dpi=300, width=12, height=12)

TSNEPlot(T_cells_0, group.by ="Treatment", cols= c("#CFBAE1", "#5C7D9D","#A4DEF9"), pt.size=0.8)
ggsave(file = "tSNE_0 - by treatment.png", dpi=300, width=12, height=12)
```
...
ggsave(file = "features_T_0.png", dpi=300, width=10, height=6.67)

# Remove clusters 11 (non-T cells)
T_cells= subset(x=T_cells_0, idents= c(0,1,2,3,4,5,6,7,9,10))
saveRDS(T_cells, file="//asafmadilabfs.tau.ac.il/asafmadilab$/Sarah/experiment/teva/memory/20221225 - memory - scRNA seq/R/3 groups/Objects/Teva_T_intermediate_object.rds")
```
...
ggsave(file = "Elbow plot_T.png", dpi=300, width=6, height=6)
saveRDS(T_cells, file="//asafmadilabfs.tau.ac.il/asafmadilab$/Sarah/experiment/teva/memory/20221225 - memory - scRNA seq/R/3 groups/Objects/T cells/Teva_T_3groups_before dim_object.rds")
```
...
ggsave(file = "tSNE_T.png", dpi=300, width=10, height=10)

TSNEPlot(T_cells, group.by ="Treatment", cols= c("#CFBAE1", "#5C7D9D","#A4DEF9"), pt.size=0.5)+
  theme(legend.text=element_text(size=20))
ggsave(file = "tSNE - T - by treatment for teva .png", dpi=300, width=10, height=10)
```
...
write_csv(Top100Markers_T, "//asafmadilabfs.tau.ac.il/asafmadilab$/Sarah/experiment/teva/memory/20221225 - memory - scRNA seq/R/3 groups/Excels/memory_T cells_DE genes.csv")
...
ggsave(file = "T cells markers - heatmap.png", dpi=300, width=12, height=20)
saveRDS(T_cells, file="//asafmadilabfs.tau.ac.il/asafmadilab$/Sarah/experiment/teva/memory/20221225 - memory - scRNA seq/R/3 groups/Objects/T cells/Teva_T_intermediate_object.rds")
```
